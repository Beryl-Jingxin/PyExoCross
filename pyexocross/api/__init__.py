...
- ``pyx.conversion(...)``           -- ExoMol-to-HITRAN or HITRAN-to-ExoMol
- ``pyx.partition_functions(...)``   -- Partition function calculation
- ``pyx.specific_heats(...)``       -- Specific heat calculation
- ``pyx.cooling_functions(...)``    -- Cooling function calculation
- ``pyx.lifetimes(...)``            -- Radiative lifetime calculation
- ``pyx.oscillator_strengths(...)`` -- Oscillator strength calculation
- ``pyx.stick_spectra(...)``        -- Stick spectra calculation
- ``pyx.cross_sections(...)``       -- Cross section calculation
"""
import os
...
def _ensure_logging(inp_filepath=None, logs_path=None):
    ...
    """
    import src.base.log as _log_mod
    ...
    if inp_filepath is not None:
        ...
        if log_dir:
            from src.base.utils import ensure_dir
            ensure_dir(log_dir + '/')
        log_path = logs_path
    else:
