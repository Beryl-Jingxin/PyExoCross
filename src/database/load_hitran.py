...
def read_parfile(read_path):
    ...
    if not os.path.exists(read_path):
        raise ValueError('The input file ' + read_path + ' does not exist.')
...
def read_hitran_pf(T_list):
    ...
    Q_col_name = ['T', 'Q']
    try:
        pf_df = pd.read_csv(read_path.replace('par', 'txt'), sep='\\s+', names=Q_col_name, header=None)
    except:
